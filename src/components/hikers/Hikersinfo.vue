    ...
    <v-container fluid grid-list-lg>
      <v-layout row wrap>
        <v-flex xs12 sm6 v-for="(hiker, i) in hikers" :key="i">
           <v-card :style="{'border-radius': '5px'}" to="/Edit-Hiker-info">
             <v-list two-line >
               <v-list-tile >
                 <v-list-tile-content>
                   <v-list-tile-title class="body-2">{{ hiker.name }}</v-list-tile-title>
...
export default {
  data: () => ({
    hikers:[
      {name:'Bouras Amine',idtype:'Passport',id:'152499721'},
      {name:'Sahroui Delhim',idtype:'Passport',id:'188401721'},
      {name:'Xu Yue',idtype:'Chinese ID',id:'18840172152158514879821'}
    ]
  })
}
</script>
